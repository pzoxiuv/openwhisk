      ...
      if (runBuffer.isEmpty || isResentFromBuffer) {
        ...
        val createdContainer =
          ...
          if (hasPoolSpaceFor(busyPool, r.action.limits.memory.megabytes.MB)) {
            ...
            logging.info(this, s"ZZZZZZ ${r.msg}")
            val unlockedArgs =
              ContainerProxy.unlockArguments(r.msg.content, r.msg.lockedArgs, ParameterEncryption.singleton)
            val (env, parameters) = ContainerProxy.partitionArguments(unlockedArgs, r.msg.initArgs)
            ...
            val dockerImage = parameters.getFields("dockerImage") match {
              case Seq(spray.json.JsString(image)) => image
              case _ => ""
            }
            logging.info(this, s"GOT dockerImage ${dockerImage}")
...
//	Maybe also include size of container image?
              ...
              .orElse(
                ...
                if (hasPoolSpaceFor(busyPool ++ freePool, r.action.limits.memory.megabytes.MB)) {
                  takePrewarmContainer(r.action, f3SeqId, mountPath, dockerImage, runtimeClass)
                    ...
                    .orElse {
                      val container = Some(createContainer(r.action.limits.memory.megabytes.MB, f3SeqId, mountPath, dockerImage, runtimeClass), "cold")
  ...
  def createContainer(memoryLimit: ByteSize, f3SeqId: String, mountPath: String, dockerImage: String, runtimeClass: String): (ActorRef, ContainerData) = {
    val ref = childFactory(context)
    val data = MemoryData(memoryLimit, f3SeqId, mountPath, dockerImage, runtimeClass)
   ...
   */
  def takePrewarmContainer(action: ExecutableWhiskAction, f3SeqId: String, mountPath: String, dockerImage: String, runtimeClass: String): Option[(ActorRef, ContainerData)] = {
    ...
    prewarmedPool.toSeq
      ...
      .find {
        case (_, PreWarmedData(_, `kind`, `memory`, `f3SeqId`, `mountPath`, `dockerImage`, `runtimeClass`, _, _)) => true
        case _                                             => false
      }
      .map {
        case (ref, data) =>
...
object ContainerPool {
  ...
  protected[containerpool] def schedule[A](action: ExecutableWhiskAction,
                                           ...
                                           mountPath: String,
                                           dockerImage: String,
                                           runtimeClass: String): Option[(A, ContainerData)] = {
    idles
      .find {
        case (_, c @ WarmedData(_, `invocationNamespace`, `action`, `f3SeqId`, `mountPath`, `dockerImage`, `runtimeClass`, _, _, _)) if c.hasCapacity() => true
        case _                                                                                   => false
      }
      .orElse {
        idles.find {
          case (_, c @ WarmingData(_, `invocationNamespace`, `action`, _, `f3SeqId`, `mountPath`, `dockerImage`, `runtimeClass`, _)) if c.hasCapacity() => true
          case _                                                                                 => false
        }
      }
      .orElse {
        idles.find {
          case (_, c @ WarmingColdData(`invocationNamespace`, `action`, _, `f3SeqId`, `mountPath`, `dockerImage`, `runtimeClass`, _)) if c.hasCapacity() => true
          case _                                                                                  => false
        }
      }
  }
        ...
        config.reactive
          .map { c =>
            val expiredPrewarmedContainer = prewarmedPool.toSeq
              .filter { warmInfo =>
                warmInfo match {
                  case (_, p @ PreWarmedData(_, `kind`, `memory`, "", "/var/data/", "", "runc", _, _)) if p.isExpired() => true
                  case _                                                                  => false
                }
              }
              .sortBy(_._2.expires.getOrElse(now))
  ...
  def increasePrewarms(init: Boolean,
    ...
    prewarmConfig.map { config =>
      ...
      val runningCount = prewarmedPool.count {
        ...
        case (_, p @ PreWarmedData(_, `kind`, `memory`, "", "/var/data/", "", "runc", _, _)) => true
